...
void VisibleSet::Copy(RenderMesh* meshes) {
    auto size = visible_set.size();
    for (size_t i = 0; i < size; i++) {
        auto inMesh = visible_set[i];
        auto outMesh = &meshes[i];
        outMesh->tex = inMesh->tex;
        outMesh->transform = inMesh->transform;
        outMesh->verts = inMesh->verts;
        outMesh->vBuffer = inMesh->vBuffer;
        outMesh->faces = inMesh->faces;
        outMesh->iBuffer = inMesh->iBuffer;
        outMesh->hasalpha = inMesh->hasalpha;
    }
}
